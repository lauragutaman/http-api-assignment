...
const statusMessages = {
    success: { code: 200, message: 'Successful Reponse' },
    badRequest: { code: 400, message: 'Missing valid query paramter set to ture', id: 'badRquest' },
    badRequestOk: { code: 200, message: 'Request has the required parameter' },
    unauthorizedMissing: { code: 401, message: 'Missing loggedIn query paramter set to yes', id: 'unauthorized' },
    unauthorizedOk: { code: 200, message: 'You have successfully viewed the content.' },
    forbidden: { code: 403, message: 'You do have access to this content.', id: 'internalError' },
    notImplemented: { code: 501, message: 'A get request for this page has not been implemented yet, check again later.' },
...
const handleBadRequest = (req, res, query) => {
    if (query.valid === 'true ') {
        ...
        Response(req, res, s.code, s.message, s.id);
    }
};
